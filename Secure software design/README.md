...
- [Course 2. Software Design as an Abstraction](#course-2-software-design-as-an-abstraction)
    - [Week 1](#week-1)
        ...
        - [Вид с высоты птичьего полета](#%D0%B2%D0%B8%D0%B4-%D1%81-%D0%B2%D1%8B%D1%81%D0%BE%D1%82%D1%8B-%D0%BF%D1%82%D0%B8%D1%87%D1%8C%D0%B5%D0%B3%D0%BE-%D0%BF%D0%BE%D0%BB%D0%B5%D1%82%D0%B0)
            - [Additional Resource: Designing Abstractions](#additional-resource-designing-abstractions)
    - [Week 2](#week-2)
        - [User Interface Design](#user-interface-design)
            - [Additional Resource](#additional-resource)
                - [7 нерушимых законов UI](#7-%D0%BD%D0%B5%D1%80%D1%83%D1%88%D0%B8%D0%BC%D1%8B%D1%85-%D0%B7%D0%B0%D0%BA%D0%BE%D0%BD%D0%BE%D0%B2-ui)
        - [User Experience](#user-experience)
    - [Week 3](#week-3)
        - [Backend and Design](#backend-and-design)
        - [Database design](#database-design)
            - [Additional Resource: Небольшая заметка на тему "Для чего нужна БД, если я просто могу хранить данные в файлах"](#additional-resource-%D0%BD%D0%B5%D0%B1%D0%BE%D0%BB%D1%8C%D1%88%D0%B0%D1%8F-%D0%B7%D0%B0%D0%BC%D0%B5%D1%82%D0%BA%D0%B0-%D0%BD%D0%B0-%D1%82%D0%B5%D0%BC%D1%83-%D0%B4%D0%BB%D1%8F-%D1%87%D0%B5%D0%B3%D0%BE-%D0%BD%D1%83%D0%B6%D0%BD%D0%B0-%D0%B1%D0%B4-%D0%B5%D1%81%D0%BB%D0%B8-%D1%8F-%D0%BF%D1%80%D0%BE%D1%81%D1%82%D0%BE-%D0%BC%D0%BE%D0%B3%D1%83-%D1%85%D1%80%D0%B0%D0%BD%D0%B8%D1%82%D1%8C-%D0%B4%D0%B0%D0%BD%D0%BD%D1%8B%D0%B5-%D0%B2-%D1%84%D0%B0%D0%B9%D0%BB%D0%B0%D1%85)
    - [Week 4](#week-4)
        - [Database design 2](#database-design-2)
            - [Additional Resource](#additional-resource)
...
##### Additional Resource: [Designing Abstractions](http://web.stanford.edu/~ouster/CS349W/lectures/abstraction.html)
...
- самая большая проблема это управление сложностью:
    - технологии позволяют нам строить системы большего размера, чем мы способны понять
...
Дизайн как абстракция помогает исправить ошибки до момента реализации системы. Особенно это верно в отношении разработки пользовательского интерфейса.
Различные экраны, с которыми оперирует пользователь системы не должны радикально отличаться друг от друга: отличается содержимое, но не способ контроля и управления со стороны пользователя - иначе пользователю привыкшему к одной структуре и стилю придется снова привыкать к структуре отличного от знакомого экрана.

Довольно сложным аспектом в проектировании UI является графика: как правильно должны сочетаться стили различным элементов, как должны располагаться изображения и графические элеметны и т.д.
Например, при работе над мультиязычным интерфейсом придется также учитывать и культурные отличия: ведь в одной культуре цвета могут интепретироваться совсем отлично от другой.

То, как продукт будет выглядеть для пользователя и как он будет себя вести, закладывается принципами дизайна при работе над UI.

Именно поэтому добавить какой то экран, кардинально отличающийся от тех, что уже существуют в системе, может быть сложным (и дорогим).

Еще один аспект UI дизайна связан с безопасностью: работа с вводом пользовотеля в различных формах логина\регистрации и т.д. налагает ответсвенность - необходимо предусмотреть защиту от злонамеренной атаки (например иньекции sql кода) или ошибок самого пользователя (drug'n'drop большого файла в поле ввода).

Для разработки безопасного интерфейса единые принципы работы над UI, которым будут следовать все разработчики системы, также жизненно необходимы.

##### Additional Resource

###### [7 нерушимых законов UI](https://99designs.com/blog/tips/7-unbreakable-laws-of-user-interface-design/)

1. избегать элементов с неясным назначением
2. пользователь должен понимать предпочтительное действие в данной ситуации
3. элементы управления объектом должны быть рядом с этим объектом
4. разработчик должен понимать, что пользователь редко будет менять настройки по умолчанию
5. пользователь скорее всего сделает что-то, если его об этом попросят, но вряд ли он это сделал бы самостоятельно
6. пользователь чувствует себя увереннее, когда получает четкую и постоянную обратную связь
7. сложные комплексные действия необходимо разбивать на более простые маленькие шаги - этим повышается вероятность успеха завершения действия пользователем

#### User Experience

Лектор рассуждает о том, что хотя и совсем недавно UX не был важен, но уже сейчас плохой UX(сложное меню, краши и т.д.) может существенно снизить прибыли приложения.

2 причины, по которым проблемы с UX существуют:
1. разработчики не знают о проблеме
2. разработчики знают о проблеме, но нет какой то существенной мотивации исправлять ее

**Причина сложности хорошего UX: когда разработчики пишет набор UI экранов, он точно знает, что должно прозойти на каждом из них, но пользователь - нет. И разработчику тяжело это представить**

UX очень субъектнивная область: разные пользователи с одним и тем же UI будут иметь разный UX.

В этом его отличии от UI - UI объективен, он определяется цветами, расположением элементов и т.д. UI можно протестировать авто тестом.

**UX же определяется целевой аудиторией и должен проектироваться и тестироваться с расчетом на нее.**

### Week 3

#### Backend and Design

![UI interaction with backend](content/backend.jpg "UI interaction with backend")

Backend собирает данные, обрабатывает и отдает результат.
Backend не имеет пользователя - только клиента.

Как правило, бэкенд не должен падать. Если же происходит такая ошибка, выполнение после которой невозможно, сообщение о ней должно быть максимально информативным.
> не понимаю, почему лектор об этом говорит только в контексте бэкенда - это верно также и для UI

А для этого необходимо понимание, где такая ошибка возможна и это делает возможным тесты на проверку появления этой ошибки.
Желательно иметь набор тестов, которые 100% покрывают код (это означает, что каждая ветка кода выполнится в теста хотя бы один раз).

Тестирование интерфейсов (с помощью некоторой среды, которая позволяет собирать данные о активности пользователя):
- сбор данных от пользователя
- преобразование в тест кейсы
- запуск тестов для проверки
- включение тестов в автоматическое регрессионное тестирование

И когда происходит ошибка, достаточно сохранить последнюю историю пользователя, а затем воспроизвести ее.

В случае бэкенда также необходимо иметь встроенную систему логирования, которая позволит обеспечить регрессионое тестирование.

#### Database design

Базы данных созданы для того, чтобы хранить данные: выполнять максмимально эффективно операции **create**, **read**, **update**, **delete**.
Часто они используют вспомогательные файлы для того, чтобы хранить данные таблиц. Физические странциы памяти преднамеренно так устроены, что добавление данных происходит в начало файла.
![DB tables and files](content/db_tables_files.jpg "DB tables and files")

Базы данных спроектированы таким образом, чтобы обслуживать запрос многих клиентов в один момент времени.

Некоторые БД позволяют использовать транзакции. Они необходимы, когда несколько событий, несколько действий на разных машинах должны быть либо выполнены все, либо не выполнены совсем.

**Two phase commit (двухфазный протокол фиксации):**
1. Множество действий подготовлено и отправлено на множество машин
2. Каждая машина подтверждает, что предварительно выполнила нужные действия
3. Если все машины ответили исходной машине (которая породила действия), то
    - исходная машина дает сигнал COMMIT
    - все остальные задействованные машины фиксируют предварительный результат как финальный
4. При ошибке\таймауте
    - исходная машины дает сигнал ROLLBACK
    - все остальные машины откатывают предварительный результат

Реляционные БД хранят данные в виде таблиц:

![Relational DB tables](content/db_tables.jpg "Relational DB tables")

Для данного примера есть 3 способа поставить в соответствие студентам те уроки, которые они посещают:

![Class association 1](content/db_tables.jpg "Первый способ")

Очевидный недостаток: некоторые уроки будут брать сотни студентов, другие же лишь несколько и колонки остальных в таблице всегда будут пустовать.

![Class association 2](content/db_junction_table.jpg "Второй способ")

Таблицы в реляционных БД имеют специальное поле-индекс **primary key**. В примере со студентами это *StudentId*, который:
- должен быть уникальным
- може быть != null

Аналогично, **CourseNum** является primary key.

##### Additional Resource: [Небольшая заметка на тему "Для чего нужна БД, если я просто могу хранить данные в файлах"](https://arstechnica.com/information-technology/2013/05/why-use-a-database-instead-of-just-saving-your-data-to-disk/)
...
Лектор упоминает здесь 3НФ, но не дает ей определения. Говорится лишь, что вся соль 3НФ в том, что данные всегда в одном месте и если нужно получить набор данных нужно выполнить некоторую сортировку и слияние:

![Third normal form example](content/db_third_normal_form.jpg "Пример третьей нормальной формы")
> вот [здесь](https://habr.com/ru/post/254773/) краткая выжимка по нормализации

Несмотря на то, что 3НФ эффективна, она может быть громоздка. И в проектах, где используется БД, является хорошей практикой описать те случаи, когда стоит избегать 3НФ.

Например, следует избегать 3НФ в случае системы заказов:

![Third normal form avoiding example](content/db_3nf_orders_ex.jpg "Пример, когда стоит избегать третьей нормальной формы")

> здесь проблема в том, что если следовать 3НФ, то при выводе информации о заказе адрес будет браться из таблицы Customers (т.е. всегда будет выбран текущий адрес, а не тот, на который была доставка)

В этом примере поможет **денормализация** и переход ко 2НФ путем дублирования данных о адресе доставки:

![Second normal form example](content/db_second_normal_form.jpg "Дублирование при второй нормальной форме")

Еще одна причина денормализации может быть оптимизация JOIN запросов (которые описывают как данные одной таблицы соединяются с данными из другой таблицы).

Далее даюся вольные определения нормальных форм:
- 1НФ: все строки в таблице имеют одинаковое количество столобцов (значения некоторых может быть null)
- 2НФ: один из столбцов является primary key и все остальные столбцы предоставляют данные по этому ключу
- 3НФ: зависимость между не ключевыми полями отсутствует (каждый не ключевой атрибут нетранзитивно зависит от первичного ключа)
- 4НФ: строка таблицы содержит только один аттрибут ключа
- 5НФ: 4НФ таблицы содержат отношения аттрибутов ключа

##### Additional Resource

###### [1. Learn about the First Normal Form and Database Design](https://www.essentialsql.com/get-ready-to-learn-sql-8-database-first-normal-form-explained-in-simple-english/)
Хороший пост о 1ой нормальной форме.


###### [2. Database - Second Normal Form (2NF)](https://www.tutorialspoint.com/sql/second-normal-form.htm)
Не самый хороший пост о 2ой нормальной форме. Лучше вышеупомянутый пост с хабра или [этот](https://www.essentialsql.com/get-ready-to-learn-sql-10-database-second-normal-form-explained-in-simple-english/)


#### Database design 3 and Overall Database Security

Безопасноть БД зависит не только о проектировании схемы - это вопрос всестороннего анализа от путей записи в БД до получения данных из БД (sql иньекции, дефолтные пароли и т.д.).

![Безопасноть БД](content/db_security_1.jpg "Безопасность БД")

Для различных вариантов доступа в БД следует продумывать вопросы безопасности:
- пользователи, которые имеют прямой доступ к БД (пользователи\администраторы): следует заводить отдельную учетную запись на каждого пользователя, с уникальным периодически обновляемым паролем. Прав учетной записи должно быть достаточно для выполнения своих задач, но не более.

- пользователи, которые имеют опосредованный (через разрабатываемое приложение) доступ в БД и приложение подключается к БД с помощью собственной учетной записи: тот, кто проектирует БД, должен позаботиться о том, чтобы приложение имело достаточно прав
...
Также при анализе безопаснасти системы не стоит забывать про то, как развертывается БД.
...
Книга "Design patterns" банды четырех стала поворотным пунктом в истории разработки ПО. Также справедливо будет сказать, что принципы, описанные в книге стали сегодня даже более актуальны, чем в 1995.

В 1970х программа представляла собой просто список команда: считать данные, что то добавить, сохранить.
Программы были хорошо продуманы, но написаны на языках, которые почти не подразумевали структуры.

В 1979 году вышла книга Christopher Alexander "The Timeless Way of Building", в которой различные шаблоны рассмотривались приминительно к архитектуре и строительства. Данная книга к 90м оказала огромное влияние на разработку ПО.

Каждый шаблон описывает проблему, которая регулярно возникает при разработке и решение этой проблемы таким образом, чтобы его можно было переиспользовать.

Хотя какие то шаблоны из этой книги уже и устарели, но сама идея того, что для сложных проектов дизайн - это повторение уже придуманных ранее шаблонов, актуальна.

> [хороший справочник по паттернам](https://refactoring.guru/ru/design-patterns)

![Развитие ЯП](content/patterns_1.jpg "Развитие ЯП")
